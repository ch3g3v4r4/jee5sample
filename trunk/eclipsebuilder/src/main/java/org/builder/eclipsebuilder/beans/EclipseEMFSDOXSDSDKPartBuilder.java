...
public class EclipseEMFSDOXSDSDKPartBuilder extends PartBuilderHelper implements PartBuilder {
    ...
    public void build(EclipseBuilderContext context) throws Exception {
        ...
        logger.info("Looking for the Eclipse EMF-SDO-XSD SDK hyperlink.");
        String[] downloadLinkAndChecksumLink = getDownloadAndChecksumLinks(downloadPage, "emf-sdo-xsd-SDK", context.getBuildType());
        String downloadLink = downloadLinkAndChecksumLink[0];
        String checksumLink = downloadLinkAndChecksumLink[1];
        logger.info("Eclipse EMF-SDO-XSD SDK hyperlink: " + downloadLink + "; checksum link:" + checksumLink);
