...
public class JTATransaction implements Transaction {
    ...
    public JTATransaction(JDBCContext jdbcContext,
            TransactionFactory.Context transactionContext) {
        this.jdbcContext = jdbcContext;
        this.transactionContext = transactionContext;

        log.debug("Looking for UserTransaction");
    ...
    public void begin() throws HibernateException {
        ...
        /*
         * if (newTransaction) { // don't need a synchronization since we are
         * committing // or rolling back the transaction ourselves - assuming //
         * that we do no work in beforeTransactionCompletion() synchronization =
         * false; }
         */

        boolean synchronization = jdbcContext
    ...
    public void commit() throws HibernateException {
        ...
        if (newTransaction) {
            try {
                ...
                commitFailed = true; // so the transaction is already rolled
                                        // back, by JTA spec
    ...
    private void afterCommitRollback() throws TransactionException {
        ...
        if (callback) { // this method is a noop if there is a Synchronization!
            ...
            if (!newTransaction) {
                log
                        .warn("You should set hibernate.transaction.manager_lookup_class if cache is enabled");
            }
            int status = NULL;
            try {
                status = ut.getStatus();
                 ...
                 * status!=Status.STATUS_ROLLEDBACK) { log.warn("Transaction not
                 * complete - you should set
                 * hibernate.transaction.manager_lookup_class if cache is
                 * enabled"); //throw exception?? }
                 */
                jdbcContext.afterTransactionCompletion(
                        status == Status.STATUS_COMMITTED, this);
            }
    ...
    public void registerSynchronization(Synchronization sync)
        ...
        if (getTransactionManager() == null) {
            ...
            try {
                ...
                throw new TransactionException(
                        "could not register synchronization", e);
            }
        }
    }
    ...
    private void closeIfRequired() throws HibernateException {
        boolean close = callback && transactionContext.shouldAutoClose()
    ...
    public void setTimeout(int seconds) {
        try {
            ...
            throw new TransactionException("could not set transaction timeout",
                    se);
        }
    }
