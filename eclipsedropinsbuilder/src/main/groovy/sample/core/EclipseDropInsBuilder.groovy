...
//    }
    public void build() {
        ...
        // working firectory
        def workDir = new File("/tmp").absolutePath
        ...
        ant.unzip (dest: workDir + "/original") { fileset(dir: workDir){ include (name: "*.zip")} }

        // Define variables
        def url = ""
        def featureId = ""
        def originalEclipseDir = workDir + "/original/eclipse"
        def eclipseDir = workDir + "/eclipse"
        def pluginsHomeDir =  workDir + "/dropins"
        def pluginTargetDir = ""

        // AnyEditTools
        url = "http://andrei.gmxhome.de/eclipse/"
        featureId = "AnyEditTools.feature.group"
        pluginTargetDir =  pluginsHomeDir + "/AnyEditTools"
    ...
    void copyPlugin(ant, url, featureId, originalEclipseDir, eclipseDir, pluginTargetDir) {
        ...
        ant.copy(todir: pluginTargetDir + "/features") {
            fileset(dir: eclipseDir + "/features", includes: "**/*") {
                present (present: "srconly", targetdir: originalEclipseDir + "/features")
            }
        }
        ant.copy(todir: pluginTargetDir + "/plugins") {
            fileset(dir: eclipseDir + "/plugins", includes: "**/*") {
                present (present: "srconly", targetdir: originalEclipseDir + "/plugins")
            }
        }
    }
}
