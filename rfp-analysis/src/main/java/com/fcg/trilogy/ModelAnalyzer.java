...
public class ModelAnalyzer {
	private static String[] match2Project = {
		"^DOS", "DOS",
		"^A7", "A7",
		"^A9000", "A9000",
		"^AFVP", "AFVP",
	...
	public static void analyze(RFPManager manager) throws Exception {
		...
		for (Iterator<String> it = serviceTypes.iterator(); it.hasNext();) {
			...
			row = sheet.createRow(rowIndex++);
			cell = row.createCell((short) 0);
			cell.setCellValue(new HSSFRichTextString(serviceType));
			colIndex = 1;
			for (Iterator<Date> it2 = dates.iterator(); it2.hasNext();) {
				Date date = it2.next();
				cell = row.createCell(colIndex++);
				cell.setCellValue(countRFPWithDateAndType(all, date,
						serviceType));
			}
		}
	...
	private static int countRFPWithDateAndType(Collection<RFP> all, Date date,
			String serviceType) {
		int count = 0;
		for (Iterator<RFP> it = all.iterator(); it.hasNext();) {
			RFP rfp = (RFP) it.next();
			if (date.equals(rfp.getDate())
					&& serviceType.equals(rfp.getServiceType())) {
				count++;
			}
		}
		return count;
